...
async function getServerDetails(serverId) {
  ...
  const [serverRes, resourcesRes] = await Promise.all([
    fetch(`${BASE_URL}/api/client/servers/${serverId}`, { headers }),
    fetch(`${BASE_URL}/api/client/servers/${serverId}/resources`, { headers }),
  ]);

  if (!serverRes.ok) return null;
  const server = (await serverRes.json())?.attributes || {};
  const resources = resourcesRes.ok ? ((await resourcesRes.json())?.attributes || {}) : {};
  const limits = server.limits || {};
  const current = resources.resources || {};

  return {
    name: server.name || null,
    description: server.description || null,
    node: server.node || null,
    allocation: server.relationships?.allocations?.data?.find(a => a.attributes?.is_default)?.attributes?.alias || null,
    diskLimit: limits.disk || null,
    cpuLimit: limits.cpu || null,
    memoryLimit: limits.memory || null,
    diskUsed: current.disk_bytes || 0,
    cpuUsed: current.cpu_absolute || 0,
    memoryUsed: current.memory_bytes || 0,
    uptime: current.uptime || 0,
  };
}

module.exports = { sendPowerSignal, getServerStatus, getServerDetails };
